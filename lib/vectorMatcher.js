...
export function cosineSimilarity(vecA, vecB) {
  ...
  // Mean Centering (Pearson Correlation Coefficient)
  const meanA = vecA.reduce((s, v) => s + (v || 0), 0) / vecA.length;
  const meanB = vecB.reduce((s, v) => s + (v || 0), 0) / vecB.length;
  ...
  for (let i = 0; i < vecA.length; i++) {
    const diffA = (vecA[i] || 0) - meanA;
    const diffB = (vecB[i] || 0) - meanB;
  ...
  const pearson = dotProduct / denominator; // [-1, 1]

  // Map Pearson correlation [-1, 1] ke match score range [0.30, 0.98]
  if (pearson <= 0) {
    return Math.max(0.25, 0.45 + pearson * 0.20);
  }
  return Math.min(0.98, 0.45 + pearson * 1.15);
}
...
/**
 * Otomatis turunkan traits siswa dari 26-dimensi studentVector jika must_have_traits_met belum diisi manual.
 * @param {number[]} studentVector
 * @returns {string[]} Array of trait strings
 */
export function deriveStudentTraits(studentVector = []) {
  const traits = new Set();
  if (!Array.isArray(studentVector) || studentVector.length < 24) return Array.from(traits);

  const [R, I, A, S, E, C, math, fis, kim, bio, indo, ing, outdoor, teamwork, analytical, creative, leadership, detail, autonomy, stability, helping, innovation, prestige, socialImpact] = studentVector;

  if (analytical >= 0.4 || math >= 0.5 || I >= 0.4) {
    traits.add('logical_thinking_high');
    traits.add('problem_solving_high');
    traits.add('critical_thinking_high');
  }
  if (I >= 0.4 || math >= 0.5 || fis >= 0.5 || kim >= 0.5 || bio >= 0.5) {
    traits.add('scientific_reasoning_high');
    traits.add('research_orientation_high');
    traits.add('pattern_recognition_high');
    traits.add('abstract_reasoning_high');
  }
  if (R >= 0.4 || analytical >= 0.4 || math >= 0.5 || fis >= 0.5) {
    traits.add('spatial_reasoning_high');
    traits.add('technical_aptitude_high');
    traits.add('practical_skills_high');
    traits.add('precision_high');
    traits.add('fine_motor_skill_high');
  }
  if (S >= 0.4 || helping >= 0.4 || socialImpact >= 0.4 || teamwork >= 0.4) {
    traits.add('empathy_high');
    traits.add('high_stress_tolerance');
    traits.add('stress_tolerance_high');
    traits.add('service_orientation_high');
    traits.add('patience_high');
  }
  if (E >= 0.4 || leadership >= 0.4 || indo >= 0.5 || ing >= 0.5) {
    traits.add('leadership_potential');
    traits.add('communication_high');
    traits.add('verbal_reasoning_high');
    traits.add('risk_tolerance_high');
  }
  if (A >= 0.4 || creative >= 0.4 || innovation >= 0.4) {
    traits.add('creative_thinking_high');
    traits.add('visual_spatial_high');
    traits.add('storytelling_ability');
    traits.add('aesthetic_sensitivity_high');
  }
  if (detail >= 0.4 || C >= 0.4) {
    traits.add('detail_oriented_high');
    traits.add('precision_high');
  }
  if (outdoor >= 0.4 || R >= 0.4) {
    traits.add('physical_stamina_high');
  }
...
export function applyGatingFilter(student, prodi, finalScore, akademikFit) {
  ...
  const studentVector = parseVector(student.student_vector) || [];
  const studentTraits = (student.must_have_traits_met && student.must_have_traits_met.length > 0)
    ? student.must_have_traits_met
    : deriveStudentTraits(studentVector);
...
export function nameBasedRiasecMatch(namaProdi, studentRiasec, kodeProdi = '') {
  ...
  if (!prodiCategory) {
    return seededVariance(seed, 0.25, 0.40);
  }

  const totalScore = Object.values(scores).reduce((s, v) => s + (v || 0), 0) || 1;
  const catScore   = (scores[prodiCategory] || 0) / totalScore;

  const rank = topTraits.indexOf(prodiCategory);

  if (rank === 0) {
    // Top 1 RIASEC match: 0.60 – 0.72
    const base  = 0.60;
    const bonus = catScore * 0.06;
    const vary  = seededVariance(seed, 0, 0.05);
    return Math.min(0.75, base + bonus + vary);
  }

  if (rank === 1) {
    // Top 2 RIASEC: 0.50 – 0.62
    const base  = 0.50;
    const bonus = catScore * 0.05;
    const vary  = seededVariance(seed, 0, 0.06);
    return Math.min(0.65, base + bonus + vary);
  }

  if (rank === 2) {
    // Top 3 RIASEC: 0.42 – 0.52
    const base  = 0.42;
    const bonus = catScore * 0.04;
    const vary  = seededVariance(seed, 0, 0.05);
    return Math.min(0.55, base + bonus + vary);
  }
 ...
 */
/**
 * Helper: Parse vector jika dalam format string JSON "[0.1, 0.2, ...]" (tipe data pgvector dari Supabase REST API)
 * @param {number[]|string} vec
 * @returns {number[]|null}
 */
export function parseVector(vec) {
...
export function runMatchingPipeline(student, prodi) {
  ...
  const majorVec   = parseVector(prodi.major_vector);
  const studentVec = parseVector(student.student_vector);

  const hasMajorVector   = Array.isArray(majorVec) && majorVec.length === VECTOR_SIZE;
  const hasStudentVector = Array.isArray(studentVec) && studentVec.length === VECTOR_SIZE;
  ...
  if (hasMajorVector && hasStudentVector) {
    ...
    matchScore = cosineSimilarity(studentVec, majorVec);
    matchMode  = 'vector';
  ...
  const akademikFit = hasAkademikMinimum
    ...
    : 0.92; // optimistic assumption: prodi tidak punya minimum ketat di DB
